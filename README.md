...
书架、章节目录、逐句/全文播放器、我的与设置、登录、隐私说明及底部弹层使用统一的暖白与深绿样式。自定义导航按状态栏和微信胶囊位置留出安全区。

- 游客默认显示“体验样本”，登录后默认显示“我的书籍”；没有最近阅读记录时不展示继续学习卡片。书目、章节数、阅读位置和全文音频可用状态均来自实际数据。
- 书架支持显式刷新和加载更多；书目点击进入章节目录，继续学习恢复阅读位置但不会自动播放。
- 所有自定义按钮显式声明 `size="default"`，共用样式及组件的 `button[size="default"].类名` 选择器覆盖微信 `style: v2` 的默认宽度和自动居中。新增按钮请沿用此约定，不能只在浏览器中验证布局。
- 书籍文字封面固定宽高比例，长书名换行时不会把封面拉长。阅读页监听网络状态，页面退出时清理监听。
